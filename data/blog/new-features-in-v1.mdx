...
.highlight-line {
  @apply -mx-4 border-l-4 border-primary-500 bg-colorScheme-700 bg-opacity-50;
}

.line-number::before {
  @apply mr-4 -ml-2 inline-block w-4 text-right text-colorScheme-400;
  content: attr(line);
}
```
...
```bash
git remote add template git@github.com:caicodes/tailwindcss-templates.git
